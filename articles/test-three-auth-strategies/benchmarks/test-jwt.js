...
// * Benchmark function
const benchmarkJWT = async () => {
    console.log('🔬 Benchmarking JWT-only authentication...\n');
    ...
    try {
        await new Promise((resolve, reject) => {
            const req = http.request(`${SERVER_URL}/api/setup/jwt`, { method: 'POST', timeout: 5000 }, (res) => {
                if (res.statusCode === 200) {
                    resolve();
                } else {
        ...
        console.error('❌ Failed to setup JWT token:', err.message);
        throw err;
    }
    ...
    if (successCount === 0) {
        ...
        console.error('   Check that the JWT_SECRET in .env matches the server configuration\n');
        throw new Error('All requests failed - server may not be running or misconfigured');
    }
