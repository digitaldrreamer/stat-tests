...
<Card.Root>
    <div class="relative h-64 overflow-hidden p-4 flex items-center justify-center">
        <img src={product.thumbnail || "https://images.unsplash.com/photo-1434056886845-dac89ffe9b56?w=600&h=400&auto=format&fit=crop&q=60&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxzZWFyY2h8N3x8ZmFzaGlvbiUyMHdhdGNoZXN8ZW58MHx8MHx8fDA%3D"} alt={product.title} class="max-h-full max-w-full object-contain" />
        <Badge.Root variant="outline" class="absolute top-2 left-2">
            {product.category}
        </Badge.Root>
        <Button.Root variant="outline" size="icon" class="absolute top-2 right-2 rounded-full">
    ...
    <Card.Content class="p-4">
        ...
        <div class="flex justify-between items-center">
            ...
            <Button.Root onclick={() => onaddToCart(product)}>
                Add to Cart
            </Button.Root>
        </div>
    </Card.Content>
